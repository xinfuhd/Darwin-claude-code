...
cp "$REPO_DIR/scripts/model-router.sh"        "$CLAUDE_DIR/scripts/"
cp "$REPO_DIR/scripts/session-handoff.sh"     "$CLAUDE_DIR/scripts/"
...
echo "新功能命令："
echo "  智能模型路由："
echo "    $CLAUDE_DIR/scripts/model-router.sh analyze '任务描述'"
echo "    $CLAUDE_DIR/scripts/model-router.sh mark <模型> unavailable"
echo ""
echo "  任务交接（切换模型前后）："
echo "    $CLAUDE_DIR/scripts/session-handoff.sh wizard    # 引导式保存"
echo "    $CLAUDE_DIR/scripts/session-handoff.sh load      # 加载上下文"
echo ""
echo "  每日复盘："
echo "    $CLAUDE_DIR/scripts/daily-review.sh show         # 查看报告"
echo ""
echo "手动触发学习："
