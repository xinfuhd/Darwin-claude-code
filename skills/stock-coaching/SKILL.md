---
name: stock-coaching
description: 美股投资辅导 skill。当用户询问股票分析、估值、投资策略、技术指标、基本面分析、持仓建议等美股相关问题时触发。支持单股深度分析、行业对比、回测验证、风险评估。
---
...
```bash
pip install yfinance pandas-ta pandas numpy matplotlib 2>/dev/null | tail -1
```
...
print(f"MACD: {latest.get('MACD_12_26_9', 'N/A')}")
```
...
# --- DCF 简化版 ---
# 用自由现金流增长折现
if free_cashflow and free_cashflow > 0 and market_cap:
...
df_bt = hist_2y.copy()
df_bt['MA50'] = ta.sma(df_bt['Close'], length=50)
df_bt['MA200'] = ta.sma(df_bt['Close'], length=200)
df_bt['Signal'] = 0
df_bt.loc[df_bt['MA50'] > df_bt['MA200'], 'Signal'] = 1  # 金叉做多
...
print(f"策略收益: {total_return*100:.1f}%")
print(f"买入持有: {buy_hold_return*100:.1f}%")
```
